...
def check_event_at_right(ctx: WContext, screen: MatLike, ignore_events: set[str]) -> Optional[str]:
    """
    识别右边区域 当前是什么事件
    """
...
def check_entry_opt_at_right(ctx: WContext, screen: MatLike, ignore_events: set[str]) -> Optional[str]:
    """
    识别右边区域 当前是否有事件的入口
    """
...
def check_event_text_and_run(op: WOperation, screen: MatLike, handlers: List[EventOcrResultHandler]) -> OperationRoundResult:
    """
    识别当前事件的文本 并做出选择
    """
...
def click_empty(op: WOperation, bottom_opt_pos: Optional[MatchResult] = None) -> OperationRoundResult:
...
def click_rect(op: WOperation, status: str, rect: Rect, wait: float = 1) -> OperationRoundResult:
    """
    点击一个位置
    """
...
def get_event_text_area(ctx: WContext) -> ScreenArea:
    """
    获取事件文本区域
    """
...
def run_event_handler(op: WOperation, handler: EventOcrResultHandler, mr: MatchResult) -> OperationRoundResult:
...
def check_dialog_confirm(op: WOperation, screen: MatLike) -> Optional[str]:
    """
    是有有对话框的确认
    """
...
def check_bottom_choose(ctx: WContext, screen: MatLike) -> Optional[str]:
    """
...
def check_bottom_remove(ctx: WContext, screen: MatLike) -> Optional[str]:
    """
    底部是否有 清除
    - 侵蚀症状
    """
...
def check_full_in_bag(ctx: WContext, screen: MatLike) -> Optional[str]:
    """
    中间是否有背包已满
    """
...
def check_screen(ctx: WContext, screen: MatLike, ignore_events: set[str]) -> Optional[str]:
    """
    识别当前画面的状态
...
def check_battle_screen(ctx: WContext, screen: MatLike) -> Optional[str]:
...
def check_mission_complete(ctx: WContext, screen: MatLike) -> Optional[str]:
...
def check_in_hollow(ctx: WContext, screen: MatLike) -> Optional[str]:
...
def check_old_capital(ctx: WContext, screen: MatLike) -> Optional[str]:
    """
    旧都失物 左上角的返回
    """
    result = screen_utils.find_area(ctx=ctx, screen=screen,
