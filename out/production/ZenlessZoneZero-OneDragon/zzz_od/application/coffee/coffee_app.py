...
class CoffeeApp(WApplication):
    ...
    @node_from(from_name='传送副本', status='模拟领域')
    @operation_node(name='模拟领域')
    def combat_simulation(self) -> OperationRoundResult:
        op = SimulationField(self.ctx, self.charge_plan)
        return self.round_by_op_result(op.execute())

    @node_from(from_name='传送副本', status='无音清剿')
    @operation_node(name='无音清剿')
    def routine_cleanup(self) -> OperationRoundResult:
        op = SilentCleanup(self.ctx, self.charge_plan)
        return self.round_by_op_result(op.execute())

    @node_from(from_name='传送副本', status='讨伐强敌')
    @operation_node(name='讨伐强敌')
    def expert_challenge(self) -> OperationRoundResult:
        op = ConquerStrong(self.ctx, self.charge_plan)
        return self.round_by_op_result(op.execute())
